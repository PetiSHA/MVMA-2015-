...
namespace UnityEngine.Networking
{
    ...
    public class NetworkManagerHUD : MonoBehaviour
    {
        ...
        void startClient()
        {
            manager.networkAddress. = hostIPInputField;
            manager.StartClient();
        }
        ...
        void OnGUI()
        {
            ...
            if (!NetworkClient.active && !NetworkServer.active && manager.matchMaker == null)
            {
                if (GUI.Button(new Rect(xpos, ypos, 105, 20), "Create Host (H)"))
                {
                    manager.StartHost();
                }
                ypos += spacing;
                ...
                if (GUI.Button(new Rect(xpos, ypos, 200, 20), "LAN Server Only mort de lol"))
                {
                    manager.StartServer();
                }
                ypos += spacing;
